...
def precision_at_k(y_true_bin, probas, k=3):
    """
    y_true_bin: (n_samples, n_labels)
    probas:     (n_samples, n_labels)
    k:          number of top labels to consider

    Returns: micro precision@k
    """
    y_true_bin = np.asarray(y_true_bin)
    probas = np.asarray(probas)
    n_samples, n_labels = probas.shape

    topk_idx = np.argsort(-probas, axis=1)[:, :k]  # top k labels
    y_pred_bin = np.zeros_like(y_true_bin)
    for i in range(n_samples):
        y_pred_bin[i, topk_idx[i]] = 1
...
def recall_at_k(y_true_bin, probas, k=3):
    """
    Micro recall@k.
    """
    y_true_bin = np.asarray(y_true_bin)
    ...
    topk_idx = np.argsort(-probas, axis=1)[:, :k]
    y_pred_bin = np.zeros_like(y_true_bin)
    for i in range(n_samples):
        y_pred_bin[i, topk_idx[i]] = 1
    ...
    *,
    ...
    if args.multilabel and mlb is not None:
        ...
        if pos_mask_test.any():
            ...
            p1 = precision_at_k(y_test_pos_true_bin, pos_true_proba, k=1)
            r1 = recall_at_k(y_test_pos_true_bin, pos_true_proba, k=1)
            p3 = precision_at_k(y_test_pos_true_bin, pos_true_proba, k=3)
            r3 = recall_at_k(y_test_pos_true_bin, pos_true_proba, k=3)
    ...
    if multilabel_metrics is not None:
        (subset_output_dir / "multilabel_metrics.json").write_text(
            json.dumps(multilabel_metrics, indent=2)
        )
...
def run_training(args: argparse.Namespace) -> None:
    ...
    binary_selected_columns = multiclass_selected_columns = list(X_train_full.columns)
