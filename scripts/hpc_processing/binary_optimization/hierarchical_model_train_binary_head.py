    ...
    *,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for train_idx, valid_idx in skf.split(X, y):
            ...
            scores.append(f1_score(y_va, y_pred, average="binary", sample_weight=w_va))
...
def train_with_feature_subset(
    *,
    ...
    # ------------------------------------------------------------------
    # Binary head optimisation + training (for two-class fenotipo)
    # ------------------------------------------------------------------
    pos_mask = (
    ...
    if not pos_mask.any():
        raise ValueError("No positive samples available to train the binary head.")
    ...
    head_params, head_threshold, multiclass_study = optimise_binary_gate(
        X_train_pos,
        y_train_pos_enc,
        n_splits=args.cv_splits,
    ...
    pos_weight_sum = float(pos_weights_balanced.loc[y_train_pos_enc == 1].sum())
    neg_weight_sum = float(pos_weights_balanced.loc[y_train_pos_enc == 0].sum())
    head_scale_pos_weight = neg_weight_sum / pos_weight_sum if pos_weight_sum else 1.0
    if args.multiclass_model == "lgbm":
        multiclass_model = LGBMClassifier(
            **head_params,
            objective="binary",
            class_weight={0: 1.0, 1: head_scale_pos_weight},
            random_state=args.random_state,
            n_jobs=-1,
        )
    else:
    ...
    multiclass_model.fit(X_train_pos, y_train_pos_enc, sample_weight=pos_weights_balanced)
    ...
    if len(pos_candidates):
        ...
        pos_proba = multiclass_model.predict_proba(X_test_pos)[:, 1]
        pos_pred_bin = (pos_proba >= head_threshold).astype(int)
        pos_pred_labels = label_encoder.inverse_transform(pos_pred_bin)
        hierarchical_pred[pos_candidates] = pos_pred_labels
        hierarchical_conf[pos_candidates] = np.maximum(pos_proba, 1 - pos_proba)
        for local_idx, sample_idx in enumerate(pos_candidates):
            multiclass_proba_per_sample[sample_idx] = {
                label_encoder.classes_[0]: float(1 - pos_proba[local_idx]),
                label_encoder.classes_[1]: float(pos_proba[local_idx]),
            }
    ...
    if pos_mask_test.any():
        ...
        pos_true_proba = multiclass_model.predict_proba(X_test_pos_true)[:, 1]
        pos_true_pred_bin = (pos_true_proba >= head_threshold).astype(int)
        pos_true_pred_labels = label_encoder.inverse_transform(pos_true_pred_bin)
    ...
    summary = {
        ...
        "multiclass_params": head_params,
        "multiclass_head_threshold": head_threshold,
        "macro_f1": macro_f1,
    ...
    print(f"[{subset_name}] Multiclass best params:", head_params)
    print(f"[{subset_name}] Multiclass head threshold:", head_threshold)
