...
This script reproduces the notebook workflow:
  ...
  6. Optional multi-label mode (--multilabel) multi-hot encodes the head and trains a
     One-vs-Rest ensemble with Optuna tuning and thresholded decoding.
...
def parse_multilabel_cell(value, *, delimiter: str, negative_label: str) -> list[str]:
    ...
    if isinstance(value, (list, tuple, set)):
        ...
        if text in {"", negative_label, "()", "[]", "{}"}:
            return []
        if not text:
            return []
        # Try to literal-eval tuple/list/set strings
        if text[0] in "([{":
            try:
    ...
    for item in raw_items:
        if pd.isna(item):
            continue
        txt = str(item).strip()
        if not txt or txt == negative_label or txt in {"()", "[]", "{}"}:
            continue
        labels.append(txt)
    return labels
...
def filter_known_labels(labels: List[str], known: set[str]) -> List[str]:
    """Keep only labels present in the fitted MultiLabelBinarizer classes."""
    return [lbl for lbl in labels if lbl in known]


# ---------------------------------------------------------------------------
# Multilabel helpers (per-label LGBM + calibration + thresholds)
# ---------------------------------------------------------------------------

def train_multilabel_head_lgbm(
    X_train,
    Y_train,
    sample_weight=None,
    random_state=99,
    ...
    """
    Train one LightGBM model per label (multi-label one-vs-rest)
    with Platt scaling (sigmoid calibration).
    """
    ...
    for j in range(n_labels):
        print(f"[multilabel] Training label {j+1}/{n_labels} ...")
        y_j = Y_arr[:, j]
        # If a label never appears, store a dummy None and keep probabilities at 0.
        if np.sum(y_j) == 0:
            models.append(None)
        ...
        base.fit(X_train, y_j, sample_weight=sample_weight)

        cal = CalibratedClassifierCV(base, cv=3, method="sigmoid")
        cal.fit(X_train, y_j, sample_weight=sample_weight)
...
def predict_multilabel_proba(models, calibrators, X):
    """
    Get calibrated probabilities (n_samples, n_labels).
    """
    n_labels = len(models)
    proba = np.zeros((len(X), n_labels), dtype=float)
    for j in range(n_labels):
        if calibrators[j] is None:
            proba[:, j] = 0.0
        else:
            proba[:, j] = calibrators[j].predict_proba(X)[:, 1]
    return proba


def tune_label_thresholds(Y_true, proba, n_points: int = 101):
    """
    Tune one threshold per label to maximise F1 for that label.
    """
...
def binarize_with_thresholds(proba, thresholds):
    """
...
def multilabel_to_strings(Y_bin, class_names, delimiter="|"):
    """
    Convert a multi-hot matrix to list[str] with joined labels.
    """
    Y_arr = np.asarray(Y_bin)
    result = []
    for i in range(Y_arr.shape[0]):
        idx = np.where(Y_arr[i] == 1)[0]
        labels = [class_names[j] for j in idx]
        result.append(delimiter.join(labels))
    return result
...
def load_processed_dataframe(csv_path: Path, cols_to_delete: list, args) -> pd.DataFrame:
    ...
    if args.multilabel:
        df[args.multiclass_target] = df[args.multiclass_target].apply(
            lambda v: parse_multilabel_cell(
                v,
                delimiter=args.multilabel_delimiter,
    ...
    if args.multilabel:
        ...
        if args.multiclass_target in ["resultado_hemo", "all_cult_org"]:
            df = df.copy()
            df = df[~df[args.multiclass_target].isin(MINOR_CLASSES_TO_DROP)]
        if "fenotipo_resistencia" in df.columns and args.multiclass_target == "fenotipo_resistencia":
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for train_idx, valid_idx in skf.split(X, y):
            ...
            if model_type == "lgbm":
                ...
                cw_filtered = {cls: class_weight.get(cls, 1.0) for cls in present_classes}
...
def build_multilabel_base_estimator(
    *,
    params: Dict[str, object],
    model_type: str,
    random_state: int,
) -> object:
    """Return a binary estimator to be wrapped by OneVsRestClassifier."""
    if model_type == "xgb":
...
def optimise_multilabel_head(
    ...
    *,
    ...
    """Tune a multi-label head using One-vs-Rest with micro-F1 optimisation."""
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        scores = []
        for train_idx, valid_idx in kf.split(X, Y_array):
            X_tr = X.iloc[train_idx]
            X_va = X.iloc[valid_idx]
            Y_tr = Y_array[train_idx]
            Y_va = Y_array[valid_idx]
            w_tr = weight_series.iloc[train_idx] if weight_series is not None else None
            w_va = weight_series.iloc[valid_idx] if weight_series is not None else None

            base_model_types = ["xgb", "lgb"]
            base_params_list = [params, params]

            model = build_stacking_multilabel_estimator(
                base_model_types=base_model_types,
                base_params_list=base_params_list,
                random_state=random_state,
            )
            # OneVsRestClassifier does not accept sample_weight in this sklearn version;
            # drop it to avoid metadata-routing errors.
            model.fit(X_tr, Y_tr)
...
def build_stacking_multilabel_estimator(
    *,
    base_model_types: list[str],
    base_params_list: list[dict],
    random_state: int,
) -> OneVsRestClassifier:
    """Crea un Stacking multi-label usando OneVsRest."""

    estimators = []
    for i, model_type in enumerate(base_model_types):
        base_est = build_multilabel_base_estimator(
            params=base_params_list[i],
            model_type=model_type,
            random_state=random_state,
        )
        estimators.append((f"{model_type}_{i}", base_est))

    meta_model = LogisticRegression(max_iter=5000, n_jobs=-1)

    # StackingClassifier multi-label
    ...
    return OneVsRestClassifier(stacking_clf, n_jobs=-1)
...
def train_with_feature_subset(
    *,
    ...
    present_gate_classes = np.unique(y_train_binary_gate)
    gate_class_weight = {cls: (gate_scale_pos_weight if cls == 1 else 1.0) for cls in present_gate_classes}
    ...
    # ------------------------------------------------------------------
    def _has_multilabel(val) -> bool:
        return isinstance(val, (list, tuple, set)) and len(val) > 0
    ...
    multiclass_study = None
    multiclass_params = {}

    # New containers for multilabel head
    multilabel_models = None
    multilabel_calibrators = None
    multilabel_thresholds = None

    if args.multilabel:
        pos_mask = (y_train_binary != args.negative_label) & y_train_multiclass.apply(_has_multilabel)
        if not pos_mask.any():
            raise ValueError("No positive samples available to train the multi-label head.")
        ...
        # MultiLabelBinarizer on cleaned labels
    ...
    macro_f1 = None
    if args.multilabel and mlb is not None:
        pos_mask_test = (y_test_binary != args.negative_label) & y_test_multiclass.apply(_has_multilabel)
        if pos_mask_test.any():
            X_test_pos_true = X_test_multiclass_scaled.loc[pos_mask_test]
            y_test_pos_true_raw = y_test_multiclass.loc[pos_mask_test]
            known_set = set(mlb.classes_)
            y_test_pos_true_filtered = [
                filter_known_labels(lbls, known_set) for lbls in y_test_pos_true_raw
            ]
            ...
            micro_f1 = f1_score(
                ...
                y_pred_bin,
                average="micro",
                sample_weight=pos_base_weight,
                zero_division=0,
            )
            macro_f1_ml = f1_score(
                y_test_pos_true_bin,
                y_pred_bin,
                average="macro",
                sample_weight=pos_base_weight,
            ...
            exact_match = np.all(y_test_pos_true_bin == y_pred_bin, axis=1)
            coverage_mask = y_pred_bin.sum(axis=1) > 0
            inclusion_mask = (y_pred_bin & y_test_pos_true_bin).sum(axis=1) > 0

            def _weighted_mean(arr):
                if pos_base_weight is None:
                    return float(np.mean(arr))
                return float(np.average(arr, weights=pos_base_weight))
            ...
            multilabel_metrics = {
                ...
                "avg_labels_predicted": float(np.average(
                    y_pred_bin.sum(axis=1),
                    weights=pos_base_weight if pos_base_weight is not None else None
                )),
                "per_label_thresholds": {
                    cls: float(th)
                    for cls, th in zip(
                        mlb.classes_,
                        multilabel_thresharray.tolist() if multilabel_thresharray is not None else [],
                    )
                },
            }
    ...
    if binary_report is not None:
        reports["binary_report.txt"] = binary_report
    if hierarchical_report is not None:
        reports["hierarchical_report.txt"] = hierarchical_report
    ...
    if multilabel_metrics:
        print(
            f"[{subset_name}] Hold-out multi-label micro-F1 {multilabel_metrics['micro_f1']:.3f} | "
            f"macro-F1 {multilabel_metrics['macro_f1']:.3f} | "
            f"exact-match {multilabel_metrics['exact_match_ratio']:.3f}"
        )
    else:
...
def run_training(args: argparse.Namespace) -> None:
    ...
    binary_selected_columns = multiclass_selected_columns = X_train_full.columns.tolist()
    ...
    if args.multilabel:
        multiclass_selected_columns = list(binary_selected_columns)
        multiclass_rfecv_selector = None
        print("Multi-label mode: skipping multiclass RFECV; reusing binary-selected features.")
    else:
        pos_mask_rfecv = (
            (y_train_binary != args.negative_label)
            & y_train_multiclass.notna()
    ...
    selection_metadata = {
        ...
        "multiclass": {
            ...
            #"rfecv_support_mask": multiclass_rfecv_selector.support_.tolist() if multiclass_rfecv_selector is not None else None,
            #"rfecv_ranking": multiclass_rfecv_selector.ranking_.tolist() if multiclass_rfecv_selector is not None else None,
        },
    ...
    aggregate_summary = {
        ...
        "multiclass": {
            "selected_feature_count": len(multiclass_selected_columns),
            "selected_features": multiclass_selected_columns,
            #"support_mask": multiclass_rfecv_selector.support_.tolist() if multiclass_rfecv_selector is not None else None,
            #"ranking": multiclass_rfecv_selector.ranking_.tolist() if multiclass_rfecv_selector is not None else None,
            #"mean_test_scores": _extract_scores(multiclass_rfecv_selector) if multiclass_rfecv_selector is not None else None,
        },
        "multilabel_metrics": summary.get("multilabel_metrics"),
    }
    (output_folder / "aggregate_summary.json").write_text(
        json.dumps(aggregate_summary, indent=2, default=_json_safe)
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--multilabel",
        action="store_true",
        help="Enable multi-label mode: multi-hot encode the multiclass target and use a One-vs-Rest head.",
    )
    parser.add_argument(
        "--multilabel-delimiter",
        type=str,
        default="|",
        help="Delimiter used to split multi-label target values (default: '|').",
    )
    parser.add_argument(
        "--multilabel-threshold",
        type=float,
        default=None,
        help="Probability threshold to decide labels in multi-label mode (default: 0.15).",
    )
    parser.add_argument(
        "--binary-trials",
        "-btrials",
