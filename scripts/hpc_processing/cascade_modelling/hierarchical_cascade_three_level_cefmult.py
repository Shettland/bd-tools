...
--------
  ...
  Level 3 – resistente_cefalosporina_multi (multiclass)
    Trained on original features + Level-1 OOF sepsis probability
...
# Two-level parallelism budget:
#   MODEL_JOBS  = OpenMP / joblib threads *inside* each model fit
#   OPTUNA_JOBS = parallel Optuna trials (each gets MODEL_JOBS threads)
# Constraint: OPTUNA_JOBS * MODEL_JOBS == N_CPUS  (≤ SLURM limit)
# Tree models (LightGBM, XGBoost, RF) plateau around 8 threads; giving
# excess CPUs to trial-level parallelism is more efficient than over-threading.
...
def _build_ranking_model(model_type: str, y: pd.Series, random_state: int) -> object:
    ...
    if model_type == "lgbm":
        ...
        return CatBoostClassifier(
            ...
            auto_class_weights="Balanced", verbose=False, random_state=42, thread_count=MODEL_JOBS
        )
...
def optimise_binary_model(
    ...
    *,
    ...
    """Tune a binary classifier with Optuna maximising ROC-AUC.
...
def shap_rfecv(
    ...
    def _cv_score(features: List[str]) -> float:
        ...
        CV folds are run in parallel using joblib threads.  Each fold gets a
        deep copy of the model so there is no shared mutable state.  Thread-based
        parallelism avoids pickle overhead and works with all supported estimators.
        """
        if scoring != "roc_auc":
            raise ValueError("Only roc_auc is supported for shap_rfecv")

        def _fit_fold(train_idx: np.ndarray, val_idx: np.ndarray) -> float:
        ...
        scores = Parallel(n_jobs=OPTUNA_JOBS, prefer="threads")(
            delayed(_fit_fold)(tr, va) for tr, va in skf.split(X[features], y)
        )
        return float(np.mean(scores))
...
def run_training(args: argparse.Namespace) -> None:
    ...
    # OOF probabilities for the training set (cascade feature for Level 2)
    print("  Generating Level 1 OOF probabilities …")
    l1_oof_proba_arr = generate_oof_probas_binary(
        X_l1_train, y_sep_train_enc, l1_params, args.model_type,
        n_splits=args.cv_splits, random_state=args.random_state,
    )
    l1_oof_series = pd.Series(l1_oof_proba_arr, index=X_train.index, name="sepsis_proba")
    ...
    l1_test_series = pd.Series(l1_test_proba, index=X_test.index, name="sepsis_proba")
    ...
    # Append Level-1 cascade feature
    # Train: use OOF sepsis probas | Test: use final L1 model output
    X2_train_scaled["sepsis_proba"] = l1_oof_series.reindex(X2_train_scaled.index).values
    X2_test_scaled["sepsis_proba"] = l1_test_series.reindex(X2_test_scaled.index).values
    ...
    # OOF multiclass probas for train (cascade features for Level 3)
    print("  Generating Level 2 OOF probabilities …")
    l2_oof_proba = generate_oof_probas_multiclass(
        X2_train_scaled, y2_train_enc, l2_params, _l2_model_type,
        num_classes=num_hemo_classes,
        n_splits=args.cv_splits, random_state=args.random_state,
    )  # shape: [n_l2_train, num_hemo_classes]
    ...
    # Build index-to-position lookups for OOF alignment in Level 3
    l2_train_idx_to_pos = {idx: pos for pos, idx in enumerate(X2_train_scaled.index)}
    l2_test_idx_to_pos = {idx: pos for pos, idx in enumerate(X2_test_scaled.index)}
    ...
    num_cef_classes = len(le_cef.classes_)

    # CatBoost does not support multiclass in this setup, so fall back to lgbm
    _l3_model_type = args.model_type if args.model_type != "catb" else "lgbm"

    print(f"  Cef classes ({num_cef_classes}): {le_cef.classes_.tolist()}")
    ...
    rank_model_l3 = _build_ranking_model(_l3_model_type, y3_train_enc, args.random_state)
    ...
    l3_features = cap_features(
        ...
        args.max_features, args.random_state, _l3_model_type, rank_model=rank_model_l3
    )
    ...
    # Append Level-1 cascade feature (sepsis proba)
    X3_train_scaled["sepsis_proba"] = l1_oof_series.reindex(X3_train_scaled.index).values
    X3_test_scaled["sepsis_proba"] = l1_test_series.reindex(X3_test_scaled.index).values

    # Append Level-2 cascade features (per-class hemo probas)
    # OOF for train rows: look up each row's position in the l2_oof_proba array
    l3_train_l2_positions = np.array(
        [l2_train_idx_to_pos[idx] for idx in X3_train_scaled.index]
    )
    l3_test_l2_positions = np.array(
        [l2_test_idx_to_pos[idx] for idx in X3_test_scaled.index]
    )

    for cls_idx, cls_name in enumerate(le_hemo.classes_):
        safe_name = str(cls_name).replace(" ", "_").replace("-", "_").replace("+", "_")
        feat_name = f"hemo_proba_{safe_name}"
        X3_train_scaled[feat_name] = l2_oof_proba[l3_train_l2_positions, cls_idx]
        X3_test_scaled[feat_name] = l2_test_proba[l3_test_l2_positions, cls_idx]

    sw_l3 = compute_balanced_sample_weight(y3_train_enc, w_train.reindex(y3_train_enc.index))

    print("  Optimising Level 3 multiclass model …")
    l3_params, l3_study = optimise_multiclass_model(
        ...
        num_classes=num_cef_classes,
    )

    # Final Level 3 model, calibrated
    l3_base = build_multiclass_model(_l3_model_type, l3_params.copy(), num_cef_classes)
    ...
    l3_test_proba = l3_model.predict_proba(X3_test_scaled)  # [n_l3_test, num_cef_classes]
    l3_test_pred = l3_model.predict(X3_test_scaled)
    ...
    if y3_test_enc.nunique() > 1:
        try:
            l3_auc = float(roc_auc_score(
                y3_test_enc, l3_test_proba,
                multi_class="ovr", average="macro",
            ))
        except ValueError:
            pass  # e.g. only one class present in test set
    ...
    (l3_dir / "summary.json").write_text(json.dumps({
        ...
        "cascade_features": ["sepsis_proba"]
            + [f"hemo_proba_{c}" for c in le_hemo.classes_],
    }, indent=2))
    l3_study.trials_dataframe().to_csv(l3_dir / "optuna_trials.csv", index=False)
    l3_test_conf = l3_test_proba[np.arange(len(l3_test_pred)), l3_test_pred]
    pd.DataFrame(l3_test_proba, columns=[f"proba_{c}" for c in le_cef.classes_]).assign(
        proba=l3_test_conf, true=y3_test_enc.values, pred=l3_test_pred
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Three-level cascade: "
            "sepsis → resultado_hemo_grouped → resistente_cefalosporina_multi (multiclass)"
        )
    )
    ...
    parser.add_argument(
        "--cef-target", type=str, default="resistente_cefalosporina_multi",
        help="Column name for the Level-3 multiclass target (default: resistente_cefalosporina_multi).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Note: Levels 2 and 3 always use lgbm/xgb/rf (not catb) for multiclass. "
            "Default: lgbm."
        ),
    )
    parser.add_argument(
